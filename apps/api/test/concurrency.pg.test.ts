...
import { createParticipant } from "../src/students/service";
import { FREE_PARTICIPANT_LIMIT } from "@pacefolio/domain";
...
/* 리뷰 P1: FREE 원생 30명 상한 — 29명 상태에서 20개 동시 등록 → 정확히 1건만 성공.
   근거 잠금: createParticipant 가 academy 행 FOR UPDATE 로 등록을 학원 단위 직렬화. */
test("FREE 상한: 29명 + 20개 동시 등록 → 정확히 1건 성공 · 최종 30명", { skip }, async () => {
  const w = await setup(); // 학원 = 구독 없음 = FREE · setup 이 원생 1명 심음
  // 29명까지 채움(퇴원 아닌 재적) — setup 의 1명 포함
  for (let i = 0; i < FREE_PARTICIPANT_LIMIT - 2; i++) {
    await w.db.insert(s.participants).values({
      id: rid("p"), academyId: w.academyId, name: `원생${i}`, birth: "2018-01-01", ageLabel: "7세",
    });
  }
  const before = await w.db.select().from(s.participants).where(eq2(s.participants.academyId, w.academyId));
  assert.equal(before.length, FREE_PARTICIPANT_LIMIT - 1); // 29

  const results = await Promise.all(Array.from({ length: 20 }, (_, i) =>
    createParticipant(w.db, {
      actorUserId: rid("u"), actorRoles: ["OWNER"], academyId: w.academyId,
      name: `동시등록${i}`, birth: "2019-02-02", ageLabel: "6세",
    }, NOW()).then((r) => r.kind, () => "ERROR")));
  assert.equal(results.filter((k) => k === "OK").length, 1, "정확히 1건만 30번째 슬롯 성공");
  assert.equal(results.filter((k) => k === "UPGRADE").length, 19, "나머지 19건은 402 UPGRADE");

  const after = await w.db.select().from(s.participants).where(eq2(s.participants.academyId, w.academyId));
  assert.equal(after.length, FREE_PARTICIPANT_LIMIT, "최종 정확히 30명 — 상한 초과 없음");
});

test("FREE 상한: 서로 다른 학원 등록은 서로 직렬화되지 않음(락 범위 = 학원 행)", { skip }, async () => {
  ...
  // 두 학원 동시 등록 — 각 학원에서 1건씩 성공(교차 방해 없음)
  const [ra, rb] = await Promise.all([
    createParticipant(wA.db, { actorUserId: rid("u"), actorRoles: ["OWNER"], academyId: wA.academyId, name: "A신규", birth: "2019-02-02", ageLabel: "6세" }, NOW()),
    createParticipant(wB.db, { actorUserId: rid("u"), actorRoles: ["OWNER"], academyId: wB.academyId, name: "B신규", birth: "2019-02-02", ageLabel: "6세" }, NOW()),
  ]);
