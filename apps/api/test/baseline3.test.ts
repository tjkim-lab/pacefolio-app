...
test("오프라인 수납: 증빙 필수·부분 수납 → PARTIALLY_PAID → 잔액 수납 → PAID · 초과 422 · 멱등", async () => {
  ...
  // 세션 리뷰 P1 회귀: 부분수납 시 미납 = total − 기수납(이중계상 금지)
  const midSum = (await (await get(founder, `/academies/${academy}/billing/summary`)).json()) as {
    unpaidCount: number; unpaidKrw: number; capturedKrw: number;
  };
  assert.equal(midSum.unpaidCount, 1);
  assert.equal(midSum.unpaidKrw, 233000);   // 333,000 − 100,000
  assert.equal(midSum.capturedKrw, 100000);
  // 초과 수납 = 422
