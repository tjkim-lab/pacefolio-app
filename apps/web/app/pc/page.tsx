...
import { DASH_KPI, CAPACITY } from "./_data";
...
export default function PCDashboard() {
  ...
  const left = TODOS.filter((t) => !done[t.key]).length;

  function complete(key: TodoKey) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-[12.5px] text-ink3 font-medium">3분기 (9~11월) · 원더짐 아카데미의 데모 설정</span>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-4 gap-3">
        <div className="rounded-2xl bg-accent-strong text-white p-4">
          <div className="text-[11.5px] font-semibold text-white/80">{DASH_KPI[0].kk}</div>
          <div className="text-[22px] font-extrabold tracking-tight mt-1">{DASH_KPI[0].kv}</div>
          <div className="text-[11px] font-semibold mt-1 text-white/80">{DASH_KPI[0].kd}</div>
        </div>
        {DASH_KPI.slice(1).map((k) => (
          <div key={k.kk} className="rounded-2xl bg-surface border border-line p-4">
            ...
            <div className={`text-[11px] font-semibold mt-1 ${k.tone === "up" ? "text-brand" : "text-ink3"}`}>{k.kd}</div>
          </div>
        ))}
        <div className="rounded-2xl bg-surface border border-line p-4">
          ...
          <div className="text-[11px] font-semibold mt-1 text-danger-ink">
            {left > 0 ? "미납 5명 · 공지 미열람 6명" : "시스템이 이어서 추적 중"}
          </div>
        </div>
      </div>
