...
⚠️ **새로 만들지 않는다.** 계산 로직은 이미 존재·검증됨: `payment-engine/` (**40/40 통과** —
**상위 소스 저장소(비공개) 루트에 위치**, 이 검토용 미러에는 미포함). 원천 스펙 `PAYMENT-ENGINE-SPEC.md` 도 동일 위치.
이 문서는 그 엔진과 `packages/domain`(모델·계약)을 **정합**시킨다.

## 1. 두 계층 — 섞지 않는다
| 계층 | 위치 | 역할 |
|---|---|---|
| **계산 엔진** | `payment-engine/` (순수함수, DB없음 — 상위 저장소) | 금액을 **계산**. 일할·할인·상한·환불 바닥·상호승인 |
| **도메인 모델** | `packages/domain` (entities·state-machines·billing) | 계산 결과를 담는 **기록·계약**(ID·상태·관계·불변식) |
